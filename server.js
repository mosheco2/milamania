 ...
 * ...
 * playersByClientId: {
 * clientId: { clientId, name, teamId, isHost } // <--- שינוי: הוספנו isHost
 * },
 * ...
 * }
 */
...
function sanitizeGame(game) {
  ...
  Object.entries(game.playersByClientId || {}).forEach(([cid, p]) => {
    playersByClientId[cid] = {
      ...
      name: p.name,
      teamId: p.teamId,
      isHost: p.isHost || false, // <--- שינוי: חושף את דגל המנהל ללקוחות
    };
  });
...
async function finishRound(gameCode, options = { reason: "manual" }) {
  ...
  if (options.reason === "timer") {
    ...
    io.to("game-" + code).emit("roundTimeUp", {
      ...
      roundScore,
      teamId,
      teamName, // <--- וידוא: שם הקבוצה נשלח כאן (כבר היה קיים בקוד המקורי, אך חשוב לוודא)
...
io.on("connection", (socket) => {
  ...
  socket.on("createGame", async (data, callback) => {
    try {
      ...
      if (dbReady && pool) {
        // (DB logic omitted for brevity, same as original)
         try {
          await pool.query(
            `
  ...
  socket.on("joinGame", async (data, callback) => {
    try {
      ...
      // <--- שינוי: בדיקה האם המצטרף הוא המנהל
      ...
      game.playersByClientId[clientId] = {
        ...
        name: playerName,
        teamId: chosenTeamId,
        isHost: isHost // <--- שינוי: שמירת הסטטוס בשרת
      };
      ...
      console.log(
        `👤 Player joined: ${playerName} -> game ${code}, team ${chosenTeamId} (Is Host: ${isHost})`
      );
      ...
      callback &&
        callback({
          ...
          clientId,
          teamId: chosenTeamId,
          teamName: game.teams[chosenTeamId].name, // <--- שינוי: החזרת שם הקבוצה המלא גם כאן
          isHost: isHost // <--- שינוי: החזרת סטטוס המנהל ללקוח
        });
  ...
  socket.on("disconnect", async () => {
    try {
      ...
      for (const code of Object.keys(games)) {
        ...
        // <--- שינוי: אם המנהל מתנתק, אנחנו לא מסירים אותו מהמשחק!
        if (game.hostSocketId === socket.id) {
            console.log(`Host disconnected temporarily: ${code}`);
            // אפשר להוסיף כאן לוגיקה של "מנהל לא מחובר" אבל לא נמחק אותו
            continue;
        }
  ...
  try {
    ...
    Object.values(games).forEach((g) => {
      ...
      const players = Object.values(playersMap).map((p) => ({
        ...
        isHost: p.isHost // <--- הוספה לתצוגת אדמין
      }));
